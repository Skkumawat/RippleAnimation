...
class Rippl: UIView {
    ...
    public func animateImpact(strength: CGFloat, duration: CFTimeInterval) {
        ...
        Rippl.remove(layer: ripplLayer, afterDuration: duration + 1.5)
    }
    ...
    private func animateImpactGrowth(layer: CAShapeLayer, strength: CGFloat, duration: CFTimeInterval) {
        ...
        let newBounds1 = bounds.insetBy(dx: (bounds.width - (bounds.width * 0)) / 2,
                                       dy: (bounds.height - (bounds.height * 0)) / 2)
        let bezierPath1 = UIBezierPath.init(ovalIn: newBounds1)
        
        
        
        layer.add(Rippl.basicAnimation(keyPath: "path",
                                       value: bezierPath1.cgPath,
                                       duration: duration + 1.5,
                                       timingFunction: kCAMediaTimingFunctionEaseIn),
                  forKey: "impactGrowth1")
    }
    ...
    private static func remove(layer: CALayer, afterDuration duration: CFTimeInterval) {
        let deadline = DispatchTime.now() + DispatchTimeInterval.milliseconds(Int(duration * 700))
